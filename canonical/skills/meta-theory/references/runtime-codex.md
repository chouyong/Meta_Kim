...
This rule is not Codex-specific: it applies to every runtime with a global hook layer (Codex, Claude, Cursor, any future addition). Each runtime's `build<Runtime>GlobalHooksTemplate()` must pass absolute paths for every hook it registers, including auxiliary hooks (e.g. Medusa's enqueue/surface hooks), not just the primary spine/enforcement hooks. A relative default silently escapes `isGlobalMetaKimManagedHookCommand`'s ownership match, which then makes the merge/strip cycle misclassify that hook as user-owned and reorder it relative to the hooks that were correctly identified — this surfaces as a hook-array-order mismatch between what sync writes and what `--check` recomputes, not as a missing-hook report. When adding a new hook family to any runtime's global template builder, grep every other runtime's equivalent builder for the same parameter name before assuming one runtime's builder is complete.
