...
async function globalHookPackageSourcePath(fileName, runtimeId) {
  ...
  return canonicalHookSourcePath(fileName, runtimeId);
}
...
async function fingerprintGlobalHookSources(runtimeId) {
  ...
  for (const fileName of globalHookPackageFileNames()) {
    const filePath = await globalHookPackageSourcePath(fileName, runtimeId);
    if (!filePath) {
...
async function copyMedusaGlobalExtraAssets(dest) {
  for (const [fileName, sourceParts] of MEDUSA_GLOBAL_EXTRA_ASSETS) {
    ...
    if (!(await pathExists(sourcePath))) {
      throw new Error(`Missing canonical Medusa asset: ${fileName}`);
    }
