...
<p align="center">
  <a href="docs/images/meta-kim-live-demo.png">
    <img alt="Meta_Kim Live 작업 보드 화면: 왼쪽의 이름 있는 세션, 가운데 4열 작업 보드, AI 역할과 도구, 활동, 차단 요소, 결과물, 증거" src="docs/images/meta-kim-live-demo.png" width="100%"/>
  </a>
</p>

<p align="center"><sub>실제 Meta_Kim Live 렌더러에 대표 데모 데이터를 표시한 화면이며, 스크린샷은 작업 보드 화면입니다. 페이지는 로컬에서 실행되며 기본값은 읽기 전용입니다.</sub></p>

기본 화면은 한 장의 실행 그래프입니다. 왼쪽에는 이름 있는 세션, 캔버스에는 해당 실행의 의존 그래프가 있고, 인스펙터는 열었을 때 부차적인 증거를 보여 줍니다. 작업 항목 상태나 프로젝트 사실을 보려면 4열 작업 보드와 프로젝트/세션 리포지터리 화면을 한 번의 클릭으로 열 수 있습니다. 단계와 시간만 남은 이전 기록은 명확히 표시하고, 새 실행은 지원되는 대화 식별 정보와 더 풍부한 작업 텔레메트리를 보존합니다.

### 최근 `main`에 반영된 업그레이드

| 업그레이드 | 변경 내용 |
| --- | --- |
| 하나의 Hub에서 여러 프로젝트 관리 | 사용자 단위 로컬 Hub는 명시적으로 등록된 Meta_Kim 프로젝트와 세션만 표시하며 디스크 전체를 검색하지 않습니다. |
| 실행 그래프가 기본 화면 | 한 장의 실행 그래프가 무엇이 무엇에 의존하는지, 어느 단계가 진행 중인지, 각 노드의 담당자와 증거 유무를 답합니다. 단계 진행률, 미니맵, 맞춤/따라가기/확대·축소, 재생도 같은 캔버스에 있습니다. |
| 보드와 리포지터리 화면 | 4열 작업 보드와 프로젝트/세션 리포지터리 화면은 한 번의 클릭으로 열려 작업 항목 상태, 담당자, 사용 중인 도구, 프로젝트 사실을 보여 줍니다. |
| 런타임 간 대화 식별 | Claude Code, Codex, Cursor, OpenClaw는 지원되는 경우 새 실행에 대화 메타데이터를 연결하며, 이전 기록의 누락은 숨기지 않습니다. |
| 증거 우선·로컬 기본값 | 사용할 수 없는 텔레메트리는 명확히 표시합니다. UI는 루프백에서만 열리고 외부 자산을 불러오지 않으며 기본값은 읽기 전용입니다. |
