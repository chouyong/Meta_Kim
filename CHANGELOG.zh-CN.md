...
- **项目指令默认保留用户已有文件。** setup 流程默认保护用户自己编写的 `AGENTS.md` 和 `CLAUDE.md`，同时支持显式选择 `portable` 与 `managed` 策略。
- **Fork 专属运行时能力继续适配上游 3.0 架构。** Claude Code 与 Codex 投影继续保留 Medusa 集成、插件覆盖契约、报告提供者预算，以及共享项目根目录/脊柱状态行为。
- **纳入上游 3.0 的维护性修复。** 本版本包含 `upstream/main` 最新的记忆根目录、精选会话、文档链接和跨运行时治理更新。

### 修复

- **Windows npm 可执行脚本使用稳定的 LF shebang。** 仓库属性阻止检出时的换行转换破坏打包后的 `bin/*.mjs` 入口。
- **打包后的四运行时全局安装使用独立且有界的 Windows 安全超时。** 不可变投影包与耐久 MCP 物化链在 Windows 上可能超过通用的五分钟命令预算，因此当前版本和历史版本种子安装各自获得十分钟、超时仍失败关闭的额度，其他普通命令仍保持五分钟上限。
- **Codex 0.147 的受控运行时证据在 Windows 上保持来源准确。** 实时探针隔离显式 provider 覆盖，使用受支持的非提权 Windows sandbox，并且仅在工作区、会话、事件时间和能力 marker 精确绑定时重放当前 `file_change` 与 TUI 父子生命周期证据。
- **Claude Code 与 Codex 全局投影继续绑定稳定包并可复现。** 同步后的运行时资产继续使用稳定投影包，同时保留用户自己的配置。
