...
| DecisionOutput 合同 | 已测通 | `config/contracts/capability-gap-output-contract.json`、FR-011、`npm run meta:gap:score-candidates` | 后续把 scorecard 渲染进 AI 可读评分表和 Web/UI 面板 |
| RunStateStore / SQLite 记录 | 已测通 | `runs`、`run_events`、`capability_gaps`、`gap_decisions`、`user_feedback` 写入；`runtime_evidence_recorded` 与 `warden_writeback_dry_run_recorded` 事件可查询；`npm run meta:trend:panel` 已生成跨 run 趋势面板 | 后续只剩服务化查询接口与更多真实 run 样本 |
...
| 用户反馈闭环 | 已测通 | `npm run meta:feedback:loop`、`config/contracts/feedback-action-contract.json`、`.meta-kim/state/default/feedback-loop/latest.json`、`.meta-kim/state/default/feedback-loop/latest.zh-CN.md`；至少 12 条 correction replay、5 类 feedback action、Review / Meta-Review gate 已测通 | 后续接真实用户多轮纠错样本和服务化反馈面板 |
| 默认产品入口 | 已测通 | `npm run meta:theory:run`、`tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` | 后续把更多真实任务样本接入默认入口回放 |
| 运行数据分析 | 已测通 | `meta:theory:run` 写入 RunStateStore，并在 run artifact 中带 analytics；`npm run meta:trend:panel` 输出 decision、blocked reason、owner、reviewer score 趋势 | 后续增加真实服务化面板 |
...
| 跨 runtime 投影验证 | 部分完成 | Claude / Codex / OpenClaw 有 live pass；Claude 与 OpenClaw 全九个 meta agents shard live evidence 已测通；Cursor smoke pass；Cursor 官方文档确认 Windows 通过 WSL 安装 `cursor-agent`，且 `-p/--print` 可配 `--output-format json`；本机当前只有 `cursor` IDE launcher，未发现独立 `cursor-agent`，因此 Cursor native live 仍返回 structured blocked；runtime evidence packet 统一记录 `status`、`evidenceKind`、`failureClass`、`command`、`artifact`、`remainingAction` | 还缺本机安装或暴露可执行 Cursor Agent CLI 后的 native live pass |
| Warden 审批写回 | 已测通 | `tests/meta-theory/32-meta-theory-four-product-targets.test.mjs` 证明 `warden-approval-v0.1` 审批包、`candidate_only`、dry-run 0 写入、`approved-for-writeback` 受控写入；`file-inventory-skill-approved-writeback` 已在用户批准后真实写入当前 repo `canonical/skills/same-set-reusable-flow-for-project-file-inventor/SKILL.md` 并 `npm run meta:sync` | 后续每次新的 current repo canonical 写入仍必须先有明确 Warden approval packet |
| 用户可读产品面板 | 已测通 | `npm run meta:theory:report -- --run-id latest` 可按 runId 读取中文报告；`runReportPanelContract` 已进入 governed run artifact；`npm run meta:theory:deliverables` 可生成静态 Web/UI 面板；`npm run meta:warden:approval-panel` 与 `npm run meta:trend:panel` 已生成审批体验和跨 run 趋势证据 | 后续做交互式服务化面板 |
| AI 可读产品标准 | 已测通 | `config/contracts/ai-readable-product-standards.json`、`config/contracts/product-delivery-bundle-contract.json`、完整产品报告中的“AI 可读标准”、`ai-readable-rubric`、`ai-readable-case-pack`、`npm run meta:delivery:bundle` | 后续接真实外部 reviewer 评分回放 |

当前可以宣称的结论：

- Complete product MVP 已经在本地证明：设计、执行、验收、反馈、交付内容都有可复查证据，且能按 AI 可读标准解释。
- 还不能宣称“发布级 live/native 全 runtime 完成”，因为 Cursor Agent CLI 尚未在本机安装或暴露；后续新的长期写回仍需要 Warden 明确批准。
...
- 本地已有但 `origin/main` 仍未包含：Cursor live harness blocked contract、runtime evidence aggregator / writeback approval、P-015 到 P-021 编排质量轨、P-022 产品面板数据合同、P-012 / P-013 / P-014 / P-023 用户交付轨、P-006 / P-007 全 meta agents shard live evidence、P-024 官方 WSL candidate probe、P-025 到 P-060 并行扩展队列、P-026 / P-027 / P-028 / P-029 / P-030 / P-031 / P-032 / P-033 / P-034 / P-035 / P-036 / P-037 / P-038 / P-039 / P-040 / P-041 / P-042 / P-043 / P-044 / P-045 / P-046 / P-047 / P-048 / P-049 / P-050 / P-051 / P-052 / P-053 / P-054 / P-055 / P-056 / P-057 / P-058 / P-059 / P-060 的本地闭合证据、以及本 PRD v0.35 的状态更新。
...
| T-005 | AI 可读产品标准 | 已测通 | `config/contracts/ai-readable-product-standards.json`、`tests/meta-theory/30-capability-gap-complete-product.test.mjs`、`tests/meta-theory/34-run-deliverables.test.mjs` | 设计、执行、验收、反馈、交付内容都有人话问题、通过标准、失败标准和证据字段 |
...
| P-012 | R-010 | Web/UI 产品面板原型：按 runId 展示 GapDecision、owner、阻塞原因、升级建议和 runtime evidence | 已测通 | frontend | 可 | `npm run meta:theory:deliverables` 生成 `run-panel.html`，读取 `artifact.runReportPanelContract`，展示判定摘要、owner handoff、阻塞与审批、runtime evidence、AI 可读评分标准，测试证明不泄露本机绝对路径 |
| P-013 | R-010 | 报告可读性 review：把协议字段翻译成人话标签，保留机器字段但不让用户被字段淹没 | 已测通 | review | 可 | `readability-review.zh-CN.md` 生成字段翻译表、前后对照和验收说明，AI 可读 reviewer 可按人话标签解释每个结论 |
| P-014 | R-011 | AI 可读评分表导出：把五维标准变成外部 reviewer 可直接打分的 Markdown / JSON rubric | 已测通 | docs | 可 | `ai-readable-rubric.zh-CN.md` 与 `ai-readable-rubric.json` 覆盖设计、执行、验收、反馈、交付内容，并映射 artifact evidence |
...
P-024 是当前发布级全 runtime native live 的硬阻塞，但它不应该把后续工作压缩成一个任务。下面这些任务可以并行推进；其中一部分用于解阻 P-024，一部分用于把已经完成的 MVP 做成更接近真实产品的持续能力。这些任务不会让当前 MVP 重新变成未完成，但完成后会提高发布级证据、用户交付质量和外部 reviewer 复盘价值。

| ID | 所属轨道 | 任务 | 状态 | 建议 owner | 可子窗口并行 | 验收证据 |
|---|---|---|---|---|---|---|
| P-025 | R-008 | Cursor WSL live 安装与只读验收子窗口：在 WSL 内安装或暴露官方 `cursor-agent`，只采集 `cursor-agent --version`、help、read-only live JSON 证据 | 阻塞 | verify | 可 | 官方文档给出的安装命令是 `curl https://cursor.com/install -fsS | bash`，验证命令是 `cursor-agent --version`；本机当前 Windows 未找到 `cursor-agent`，WSL `command -v cursor-agent` 为空。通过后 `node scripts/eval-meta-agents.mjs --runtime=cursor --live` 必须返回 `summary.passed = ["cursor"]`，且无文件写入 |
| P-026 | R-008 | Cursor 官方文档 source-backed refresh：由 `meta-scout` 重新抓取官方 CLI overview / using / parameters / installation / output-format 文档，校准 P-024 的 pass / blocked 条件 | 已测通 | meta-scout | 可 | 2026-06-05 官方文档复核：CLI overview / installation / parameters / output-format 均证明 `cursor-agent`、Windows WSL 安装、`--print`、`--output-format json` 路径存在；本机缺 CLI 时仍保持 blocked；`tests/setup/eval-meta-agents.test.mjs` 锁定合同 |
| P-027 | R-008 | Cursor native success fixture：在不依赖本机真实 Cursor Agent 的情况下，构造一个成功 JSON fixture，证明 harness 能从 blocked 自动升级为 pass | 已测通 | backend | 可 | PowerShell：`$env:META_KIM_CURSOR_LIVE_SUCCESS_FIXTURE='1'; node scripts/eval-meta-agents.mjs --runtime=cursor --live` exit 0；`summary.passed = ["cursor"]`、`failureClass = "pass"`、`strictReleasePass = true`、`releaseGrade = true`；真实 P-024 仍需本机 native CLI |
| P-028 | Release closure | GitHub 差距自动报告：把本地相对 `origin/main` 的能力差距、未 push commit、PRD 状态和产品设定生成机器可复查报告 | 已测通 | docs | 可 | `npm run meta:github:gap` 生成 `.meta-kim/state/default/github-gap-report/latest.json` 与 `latest.zh-CN.md`；报告含 ahead commit 数、当前 GitHub 差距、P-024 不能宣称完成、P-026 / P-027 / P-028 / P-034 / P-036 已测通队列 |
| P-029 | R-010 | 跨 run 趋势服务化面板：把静态 run panel 扩展成可看 decision 分布、blocked 原因、owner 失败率和 reviewer 评分趋势的服务化视图 | 已测通 | frontend | 可 | `npm run meta:trend:panel` 生成 `.meta-kim/state/default/run-trend-panel/latest.json` 与 `latest.zh-CN.md`；5 类 filter、4 个 panel、5 行 reviewer score trend、blocked reason trend 和 GitHub delta 均可读；`privacyStatus = "pass"` |
| P-030 | R-011 | AI 可读 reviewer 样本回放：用近真实评审样本验证 rubric、case pack 和报告是否能被非维护者理解 | 已测通 | review | 可 | `npm run meta:reviewer:replay` 生成 `.meta-kim/state/default/product-reviewer-replay/latest.json` 与 `latest.zh-CN.md`；3 个 reviewer 样本覆盖产品用户、技术 reviewer、外部 reviewer，输出五维评分、6 条误解点与修订建议，明确 fixture pass、release-grade、GitHub complete、Warden approval 等理解风险 |
| P-031 | R-009 | 真实 Warden approved canonical writeback：在用户明确批准后，把一个低风险候选能力写入当前 repo canonical，并完成 sync / review / verification | 已测通 | meta-warden | 可 | 用户批准后执行 `file-inventory-skill-approved-writeback`，审批包 `warden-approved-file-inventory-skill-2026-06-05` 通过，`writeback = "approved-for-writeback"`、`canonicalWrites = 1`、`applyStatus = "created"`、`verificationResult.status = "pass"`；新增 `canonical/skills/same-set-reusable-flow-for-project-file-inventor/SKILL.md` 并已 `npm run meta:sync` |
| P-032 | R-008 | OpenClaw batch live 稳定性改进：排查全九 agent 批量 live timeout 的原因，决定是并发限制、超时策略还是 shard runner 改造 | 已测通 | backend | 可 | `npm run meta:openclaw:batch-stability` 生成 `.meta-kim/state/default/openclaw-batch-stability/latest.json` 与 `latest.zh-CN.md`；9 个 shard pass 证据保留，batch probe 明确 `expectedFailureClass = "timeout"`、`releaseGradeCandidate = false`、retry policy 和 remainingAction |
| P-033 | R-008 | runtime live shard 自动化矩阵：把 Claude / OpenClaw / Codex / Cursor 的 shard 命令、超时、证据文件和失败类统一成矩阵 runner | 已测通 | backend | 可 | `npm run meta:runtime:shard-matrix` 生成 `.meta-kim/state/default/runtime-live-shard-matrix/latest.json` 与 `latest.zh-CN.md`；records 覆盖 Claude / Codex / OpenClaw / Cursor，含 command、timeoutMs、artifact、expectedFailureClass、releaseGradeCandidate、remainingAction，并保留 Cursor blocked 与 fixture-only 边界 |
| P-034 | Verification | 子窗口验收包模板：为每个可并行任务生成固定的只读验收提示、允许命令、禁止动作和 PASS/FAIL 输出模板 | 已测通 | test | 可 | `npm run meta:verification:subwindows` 生成 `.meta-kim/state/default/subwindow-verification-packets/latest.json` 与 `latest.zh-CN.md`；每个 packet 都是 read-only verification，含 allowedCommands、forbiddenActions、PASS/FAIL 输出要求和主窗口合并政策 |
| P-035 | R-001/R-007/R-011 | 真实复杂输入扩展集：增加更多多类型、多同类、多依赖、多阻塞的用户任务样本，验证 Conductor 编排不会退化 | 已测通 | test | 可 | `npm run meta:gap:complex-inputs` 生成 `.meta-kim/state/default/complex-capability-gap-inputs/latest.json` 与 `latest.zh-CN.md`；10/10 pass，覆盖 research-first、multi-capability、approval-blocked、workerTask-only、create-agent、create-script、mcp-provider、same-type-repeat、multi-runtime、reviewer-feedback |
| P-036 | PRD governance | PRD 状态完整性守卫：测试 PRD 至少保留主干任务、未完成任务、GitHub 差距、并行队列和 AI 可读标准，防止后续又散成多份文档 | 已测通 | meta-prism | 可 | `tests/meta-theory/29-capability-gap-complete-product-prd.test.mjs` 与 `tests/meta-theory/35-release-closure-deliverables.test.mjs` 锁定单一 PRD、GitHub 差距、P-025 到 P-060 队列、已测通状态、AI 可读五维标准和不能宣称 P-024 完成 |
| P-037 | R-007/R-010 | 深度研究准备层产品化：把 `meta-scout` 的联网搜索、官方文档抓取、来源可信度、阻塞返回和 Thinking handoff 做成可审计 report section | 已测通 | meta-scout | 可 | `npm run meta:research:prepare` 生成 `.meta-kim/state/default/research-preparation/latest.json` 与 `latest.zh-CN.md`；4/4 case pass，覆盖 current-fact、official docs、external MCP provider、local-only、blocked paid/credential boundary；每个 `researchPreparationPacket` 都记录 searchAngles、sourceList、freshness、credibility、blockedReason、decisionImpactMap、thinkingHandoff，报告解释“研究完才编排” |
| P-038 | R-007/R-010 | 多类型能力库存浏览器：按 agent、skill、script、command、MCP、runtime tool、plugin、retrieval、dependency、workerTask-only 展示候选能力和缺口 | 已测通 | meta-artisan | 可 | `npm run meta:capabilities:browser` 生成 `.meta-kim/state/default/multi-type-capability-browser/latest.json` 与 `latest.zh-CN.md`；10/10 capability types covered，237 个候选，`skillOnly = false`，每类都有 count、topCandidates、unavailableReasons、innovationNeeded；报告明确 skill 只是能力类型之一 |
| P-039 | R-003/R-007 | 编排 DAG 可视化与依赖模拟：把 `orchestrationTaskBoardPacket` 的并行组、线性依赖、mergeOwner、阻塞点变成可读图和机器检查 | 已测通 | meta-conductor | 可 | `npm run meta:orchestration:dag` 生成 `.meta-kim/state/default/orchestration-dag/latest.json` 与 `latest.zh-CN.md`；3 个 DAG case pass，`fakeParallelismCount = 0`，fake parallelism 计数为 0，blocked node 可见，Mermaid preview 可读，节点保留 dependsOn、parallelGroup、mergeOwner、roleInstanceId、shardScope |
| P-040 | R-001/R-007 | workerTask 输出合同与返工策略：执行前定义每类 worker 的 output schema、失败返回阶段、最多重试、Review 接收条件 | 已测通 | backend | 可 | `config/contracts/worker-task-output-contract.json` 与 `npm run meta:worker:outputs` 已定义 6 类 worker output schema、required evidence、failureClass、returnToStage、maxRetries 和 Review 接收条件；报告证明 Review 不接受缺 owner boundary、缺 evidence 或 schema 不合格输出 |
| P-041 | R-002/R-011 | 真实用户纠错 replay 池：把用户在聊天里的纠正、误解、路线偏好转成可回放 fixture，验证下一轮是否少犯同类错 | 已测通 | review | 可 | `npm run meta:feedback:loop` 生成 `.meta-kim/state/default/feedback-loop/latest.json` 与 `latest.zh-CN.md`；12 条 correction replay 覆盖 decision、owner、research、parallelism、deliverable、writeback，`changedDecisionCount = 12`，每条都记录 effectOnNextRun |
| P-042 | R-002/R-010 | 反馈入口与状态闭环：在用户可读报告里加入“接受 / 纠正 / 拒绝 / 升级长期能力 / 保持一次性”的反馈入口合同 | 已测通 | frontend | 可 | `config/contracts/feedback-action-contract.json` 定义 run-state-only feedback actions；报告证明 accept / correct / reject / promote_to_long_term / keep_one_time 五类都覆盖，反馈会写入 runStateWrite，不自动改 canonical，并能显示反馈是否影响下一轮 route |
| P-043 | R-009/R-010 | Warden 审批体验面板：把 approval packet、候选写回 diff、风险、回滚、批准/拒绝结果做成用户可读审批页 | 已测通 | meta-warden | 可 | `npm run meta:warden:approval-panel` 生成 `.meta-kim/state/default/warden-approval-experience/latest.json` 与 `latest.zh-CN.md`；approval panel 显示 approvalRequired、diff preview、risk、rollback、verification owner；当前 repo canonical 写入保持 0 |
| P-044 | R-008 | runtime 解阻安装/验收手册：为 Cursor、OpenClaw、Codex、Claude 生成 read-only install/probe playbook 和失败类说明 | 已测通 | verify | 可 | `npm run meta:runtime:probe-playbook` 生成 `.meta-kim/state/default/runtime-probe-playbook/latest.json` 与 `latest.zh-CN.md`；覆盖 Windows native、WSL、IDE subcommand、PATH missing、auth missing、output-format missing 等变体；Cursor P-024 仍以真实 CLI 为准 |
| P-045 | R-010/R-011 | 产品交付包打包：把 panel、report、rubric、case pack、GitHub gap、runtime matrix 合成一个 AI 可读 product bundle | 已测通 | docs | 可 | `npm run meta:delivery:bundle` 一条命令生成 bundle manifest：`.meta-kim/state/default/product-delivery-bundle/latest.json` 与 `.meta-kim/state/default/product-delivery-bundle/latest.zh-CN.md`；bundle manifest 覆盖 panel、readability review、rubric、case pack、GitHub gap、runtime matrix、DAG report、research report、feedback report 和 bundle 自身，`fileCount = 12` |
| P-046 | R-011 | reviewer 校准样本：为外部 reviewer 准备“如何判定这个产品做对了”的正例、反例和评分样本 | 已测通 | review | 可 | `tests/meta-theory/scenarios/reviewer-calibration-samples.json` 提供 8 个 reviewer 评分样本、正/反例和提示，覆盖 research-before-orchestration、skill-only capability、fake parallelism、fixture pass as live、unauthorized writeback、GitHub gap overclaim、Warden approval confusion、mixed deliverables，并用反例说明能力不等于 skill |
| P-047 | R-007 | 真实联网研究执行层：在研究准备层之后执行 source-backed web / docs / URL fetch，并把引用、时间戳、可信度、阻塞原因写入 Thinking handoff | 已测通 | meta-scout | 可 | `npm run meta:research:execute` 生成 `.meta-kim/state/default/research-execution/latest.json` 与 `.meta-kim/state/default/research-execution/latest.zh-CN.md`；6 条研究样本覆盖 official_docs、news_version、third_party_tool、credential_blocked、network_blocked，4 条 live fetch 记录 HTTP 200、byteLength、sha256、fetchedAt，2 条 blocked 返回 Fetch |
| P-048 | R-007/R-010 | 研究证据缓存与新鲜度策略：避免每次全量搜索，同时让过期证据触发 refresh | 已测通 | backend | 可 | `config/contracts/research-execution-contract.json` 与 research execution report 记录 `fetchedAt`、`sourceType`、`freshnessPolicy`、`staleReason`；`staleRefreshCount = 1`，过期证据会 `return_to_fetch_refresh_before_thinking` |
| P-049 | R-007/R-009 | 能力创新候选沙箱：当 agent/skill/script/MCP/runtime/dependency 都不够时，只生成创新候选、风险和 ROI，不直接写长期能力 | 已测通 | meta-artisan | 可 | research execution report 输出 2 个 `innovationCandidatePacket`，含 capability type、替代路径、成本、风险、最小试验、Warden approval requirement；`canonicalWrites = 0` |
| P-050 | R-003/R-007 | DAG 串行依赖样本扩展：补足当前 P-039 edgeCount 为 0 的限制，加入 research -> capability -> execution -> review 的真实依赖链 fixture | 已测通 | test | 可 | `npm run meta:orchestration:schedule` 生成 `.meta-kim/state/default/orchestration-scheduler/latest.json` 与 `.meta-kim/state/default/orchestration-scheduler/latest.zh-CN.md`；5 个 DAG case，3 个有非空 dependsOn，9 条 edge，`orphanDependencyCount = 0`、`cycleCount = 0`、`blockedDependencyViolationCount = 0` |
| P-051 | R-003/R-007 | DAG 调度仿真与关键路径：为 workerTask 标注估算耗时、可并行度、关键路径和等待原因 | 已测通 | meta-conductor | 可 | `npm run meta:orchestration:schedule` 报告 `criticalPath`、`parallelUtilization`、`serialBottleneck`、`blockedWaitReason`；覆盖 research -> capability -> execution -> review 串行链、fanout-then-merge 并行后合并、approval blocked wait reason |
| P-052 | R-001/R-007 | workerTask 输出 schema registry：按 research、implementation、review、verification、writeback、deliverable 定义输出合同 | 已测通 | backend | 可 | `config/contracts/worker-task-output-contract.json` 覆盖 research、implementation、review、verification、writeback、deliverable 6 类；`tests/meta-theory/41-worker-task-output-report.test.mjs` 断言每类 requiredFields、requiredEvidence、failureClass、returnToStage 和 maxRetries |
| P-053 | R-001/R-007 | workerTask 返工 runner：把不合格 worker 输出送回指定阶段，记录 retry 次数、原因和停止条件 | 已测通 | backend | 可 | `npm run meta:worker:outputs` 生成 `.meta-kim/state/default/worker-task-output/latest.json` 与 `.meta-kim/state/default/worker-task-output/latest.zh-CN.md`；9 个 replay 样本全部 pass，6 accept、1 retry、1 return_to_stage、1 blocked，三类失败分别覆盖缺 evidence、owner 越界、schema 不合格 |
| P-054 | R-008/R-011 | Review / Meta-Review 双层接收门：Review 先查 worker 输出，Meta-Review 再查 Review 标准是否足够 | 已测通 | meta-prism | 可 | `npm run meta:feedback:loop` 的 reviewGate 覆盖 4 个样本：1 accept、3 reject；Review 必查 Critical、Fetch、Thinking、Execution、worker output、feedback effect、acceptance evidence；Meta-Review 可把 polish-only review 返回 Review、缺 Fetch 返回 Fetch、缺 writeback boundary 置 blocked |
| P-055 | R-002/R-010 | 用户反馈动作合同：把接受、纠正、拒绝、升级长期能力、保持一次性定义成可写入 run state 的 action schema | 已测通 | frontend | 可 | `config/contracts/feedback-action-contract.json` 定义 actor、target、reason、effectOnNextRun、writebackIntent、wardenApprovalRequired；promotion 只能 `candidate_only`，`canonicalWritesWithoutApproval = 0` |
| P-056 | R-002/R-010/R-011 | 反馈 replay 指标面板：展示纠错类型、重复缺口、下次路线是否改变、reviewer 是否看懂 | 已测通 | review | 可 | `.meta-kim/state/default/feedback-loop/latest.json` 汇总 `changedDecisionCount = 12`、`repeatGapCount = 11`、`reviewerConfusionReduced = 12`、`reviewRejectedCount = 3`；中文报告解释下一轮 route 如何改变 |
| P-057 | R-009/R-010 | Warden 审批 diff 预览与回滚演练：把候选写回目标、diff 摘要、风险、回滚命令和验证 owner 做成审批前页面 | 已测通 | meta-warden | 可 | `npm run meta:warden:approval-panel` 在临时 canonicalRoot 演练候选写入、sync、rollback、verification；`rollbackVerified = true`，`currentRepoCanonicalWrites = 0`，不动当前 canonical |
| P-058 | R-008 | runtime probe 变体矩阵：覆盖 Windows native、WSL、IDE subcommand、PATH missing、auth missing、output-format missing 等变体 | 已测通 | verify | 可 | `npm run meta:runtime:probe-playbook` 输出至少 12 个 runtime/environment variant，缺失环境为空，Cursor native 仍保持 blocked；每个 variant 都有 probeCommand、expectedFailureClass、remainingAction |
| P-059 | R-010/R-011 | 产品交付 bundle CLI：一条命令打包 panel、report、rubric、case pack、GitHub gap、runtime matrix、DAG report、research report | 已测通 | docs | 可 | `meta:delivery:bundle` 的 bundle manifest 固定 file list、audience、reviewUse、reviewerPrompt、passSignal、failSignal、privacyCheck；`privacyStatus = "pass"` 且禁止本机绝对路径泄露 |
| P-060 | R-011 | reviewer 评分校准与反例库：为“做对/没做对”的外部判断准备可评分样本、反例和 reviewer 提示 | 已测通 | review | 可 | `product-reviewer-calibration-v0.1` 输出 8 个评分样本，`missingPitfalls = []`，覆盖研究未做先编排、skill-only 误判、fake parallelism、fixture pass 冒充 live、未授权 writeback、GitHub 差距误判、Warden 审批误解、交付混写 |
...
6. 已完成 P-022 / P-012 / P-013 / P-014 / P-023：用户交付轨已有面板数据合同、静态 Web/UI、可读性 review、AI 可读评分表和案例包。下一步用户交付轨只扩展真实 reviewer 样本，不再阻塞本轮闭合。
7. 已完成 P-026 / P-027 / P-028 / P-034 / P-036：Cursor 官方文档 evidence 已刷新，Cursor success fixture 已证明 pass 分支，GitHub 差距报告和子窗口验收包已有 CLI，PRD 状态完整性已有测试守卫。P-024 仍保持真实 native live 阻塞。
8. 已完成 P-033 / P-035：runtime live shard matrix 已能统一四端命令、artifact、failureClass、remainingAction；复杂输入扩展集已有 10 条 replay，证明 Conductor 对 research-first、多能力、阻塞、一次性 workerTask 和 reviewer-feedback 类输入不退化。
9. 已完成 P-030：AI 可读 reviewer 样本回放已有 3 个近真实样本，能输出五维评分、误解点和报告/面板改写建议，补足“外部 reviewer 能不能看懂”的反馈证据。
10. 已完成 P-037：深度研究准备层已有可审计 packet 和中文报告，证明 current-fact / official-docs / external-provider / local-only / blocked-boundary 输入会在 Thinking 前先完成研究准备或返回 Fetch。
11. 已完成 P-038：多类型能力库存浏览器已有 JSON 和中文报告，按 agent、skill、script、command、MCP、runtime tool、plugin、retrieval、dependency、workerTask-only 十类展示候选、风险和是否需创新，避免把 capability 简化成 skill。
12. 已完成 P-039：编排 DAG 报告已有 JSON 和中文报告，3 个 orchestrationTaskBoardPacket case pass，blocked node 保持可见，`fakeParallelismCount = 0`，节点固定保留 dependsOn、parallelGroup、mergeOwner、roleInstanceId、shardScope。
13. 已完成 P-050 / P-051：调度仿真报告已有 5 个 DAG case、3 个依赖 case、9 条 edge，证明 research -> capability -> execution -> review 串行链、fanout-then-merge、approval blocked wait reason、criticalPath、parallelUtilization、serialBottleneck 和 blockedWaitReason 均可审计。
14. 已完成 P-040 / P-052 / P-053：workerTask 输出合同与返工报告已有 6 类 schema、9 个 replay 样本、6 accept、1 retry、1 return_to_stage、1 blocked，证明缺 evidence 会 retry，owner 越界会 blocked，schema 不合格会 returnToStage，Review 不接收无 owner/evidence/schema 的输出。
15. 已完成 P-041 / P-042 / P-054 / P-055 / P-056：反馈闭环已有 12 条 correction replay、5 类 feedback action、run-state-only 写入合同、Review / Meta-Review 双层接收门和中文报告；证明用户纠正会影响下一轮 route，重复缺口可见，reviewer 误解下降，未获 Warden approval 时 canonical 写入保持 0。
16. 已完成 P-045 / P-046 / P-059 / P-060：产品交付 bundle 已能一条命令打包 panel、report、rubric、case pack、GitHub gap、runtime matrix、DAG report、research report、feedback report，并生成 reviewer 评分校准与反例库；5 个产品 section 全覆盖，8 个评分样本覆盖全部理解误区，隐私检查 pass。
17. 已完成 P-047 / P-048 / P-049：真实联网研究执行层已有 6 条样本、4 条 live fetch、2 条 blocked boundary、1 条 stale refresh probe 和 2 个 innovationCandidatePacket；证明“研究准备”不会冒充“真实 fetch”，过期证据会回 Fetch，能力不足时只生成 candidate-only 创新候选且 `canonicalWrites = 0`。
18. 已完成 P-029 / P-032 / P-043 / P-044 / P-057 / P-058：跨 run 趋势、OpenClaw batch 稳定性、Warden 审批体验 / diff / rollback、runtime probe playbook 和变体矩阵均已有脚本、报告和测试；这些只生成证据和临时演练，不清除 Cursor native 阻塞，也不写当前 canonical。
19. 已完成 P-031：在用户明确批准后完成一次真实 current repo canonical writeback，新增项目文件用途整理 skill，审批包、写入结果、sync 和验证状态均可从 `file-inventory-skill-approved-writeback` run artifact 复查；这只证明本次低风险目标已获批写入，不代表后续 canonical 写回可跳过 Warden approval。
...
| User deliverable | P-012、P-013、P-014、P-023 | meta-warden | 已测通；`npm run meta:theory:deliverables` 生成 panel、readability review、rubric、case pack、manifest，且测试证明每类交付物独立存在 | 后续只能扩展真实 reviewer 样本，不能把某一类交付物冒充另一类 |
| Extension backlog | P-025 到 P-060 | meta-conductor | P-026 / P-027 / P-028 / P-029 / P-030 / P-031 / P-032 / P-033 / P-034 / P-035 / P-036 / P-037 / P-038 / P-039 / P-040 / P-041 / P-042 / P-043 / P-044 / P-045 / P-046 / P-047 / P-048 / P-049 / P-050 / P-051 / P-052 / P-053 / P-054 / P-055 / P-056 / P-057 / P-058 / P-059 / P-060 已测通 | P-025 需要安装或暴露 Cursor Agent CLI；P-024 解阻前不能宣称全 runtime release-grade 完成 |
...
产品族口径收束：真实联网研究、反馈闭环、审批体验、runtime probe 变体、交付打包、reviewer 校准。AI 可读标准只服务于外部 reviewer 对设计、执行、验收、反馈和交付证据的复核。
...
| R-004 Run Analytics | 已测通 | RunStateStore 查询 decision 分布、用户纠错分布、candidate 接受率、blocked 原因、repeatKey top、owner 失败率；`meta:trend:panel` 已生成跨 run 趋势报告 |
...
| R-010 用户可读 UI / 报告层 | 已测通 | `meta:theory:report`、完整产品 Markdown 报告、跨 run 趋势报告、Warden 审批体验报告可解释判定、owner、验证、升级建议、审批风险和回滚演练 |
| R-011 AI 可读产品标准 | 已测通 | `ai-readable-product-standards.json` 和报告中的五维标准：设计、执行、验收、反馈、交付内容 |
...
- 官方 evidence：2026-06-04 刷新 Cursor CLI overview / using / parameters / output-format / installation 文档；文档声明 Agent CLI、non-interactive print mode、`--output-format json`、成功时 JSON object 输出、失败时 non-zero 且无 well-formed JSON，以及 macOS / Linux / Windows WSL 可用 `cursor-agent --version` 验证。
...
- 让设计、执行、验收、反馈、交付内容都能被外部 reviewer 看懂，不依赖隐藏协议词。
...
- 必须有一个机器可读标准合同：`config/contracts/ai-readable-product-standards.json`。
- 标准必须覆盖 5 个维度：设计、执行、验收、反馈、交付内容。
- 每个维度必须包含：人话问题、通过标准、失败标准、requiredEvidence。
- 完整产品报告必须渲染“AI 可读标准”，且每个维度 status = pass。
...
| AI 可读可解释 | 设计、执行、验收、反馈、交付内容都有 Plain-language question、PASS、FAIL 和 requiredEvidence |
