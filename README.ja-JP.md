...
<p align="center">
  <a href="docs/images/meta-kim-live-demo.png">
    <img alt="Meta_Kim Live のワークボード表示：左に名前付きセッション、中央に 4 列の作業ボード、AI ロールとツール、活動、ブロッカー、成果物、証拠" src="docs/images/meta-kim-live-demo.png" width="100%"/>
  </a>
</p>

<p align="center"><sub>実際の Meta_Kim Live レンダラーを、代表的なデモデータで表示しています。スクリーンショットはワークボード表示です。ページはローカルで動作し、既定では読み取り専用です。</sub></p>

既定の表示は 1 枚の実行グラフです。左に名前付きセッション、キャンバスにその実行の依存グラフ、インスペクターは開いたときに副次的な証拠を表示します。作業項目の状態やプロジェクトの事実を見たいときは、4 列の作業ボードとプロジェクト／セッションのリポジトリ表示が 1 クリックで開きます。段階と時刻しかない旧レコードは明示され、新しい実行では利用可能な会話 ID と詳細な作業テレメトリを保持します。

### 最近 `main` に取り込まれたアップグレード

| アップグレード | 変更内容 |
| --- | --- |
| 1 つの Hub で複数プロジェクト | ユーザー単位のローカル Hub は、明示的に登録された Meta_Kim プロジェクトとセッションだけを一覧表示し、ディスク全体を走査しません。 |
| 実行グラフが既定画面 | 1 枚の実行グラフが、依存関係、進行中の段階、各ノードの担当、証拠の有無に答えます。段階進捗、ミニマップ、フィット／追従／ズーム、リプレイも同じキャンバス上にあります。 |
| ボードとリポジトリ表示 | 4 列の作業ボードとプロジェクト／セッションのリポジトリ表示は 1 クリックで開き、作業項目の状態、担当、使用中のツール、プロジェクトの事実を確認できます。 |
| ランタイム横断の会話 ID | Claude Code、Codex、Cursor、OpenClaw は対応時に新しい実行へ会話メタデータを関連付け、旧レコードの欠落は隠しません。 |
| 証拠優先・ローカル既定 | 取得できないテレメトリは「利用不可」と明示します。UI はループバックのみで、外部アセットを読み込まず、既定では読み取り専用です。 |
